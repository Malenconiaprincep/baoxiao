...
let data = xlsx.parse(fs.readFileSync('./3.xlsx')); // parses a buffer
let destPath = 'zj.xlsx';
...
function level(end, start, special) {
  ...
  if (special) {
    if (
      getTime(end) - getTime(start) >= 4 * 3600 &&
      getTime(end) - getTime(start) < 8 * 3600
    ) {
      price = 35;
    } else {
      price = 50;
  ...
  for (var i = 0; i < list.length; i++) {
    ...
    if (end && start && check(end, start, special)) {
      if (level(end, start, special)) {
        list[i][7] = level(end, start, special);
        arr.push(list[i]);
      }
    }
  }

  let originlist = parseList(destFile, 0);
  originlist = originlist.concat(arr);
  destFile[0].data = originlist;
  console.log(originlist);
  let buffer = xlsx.build([{ name: 'mySheetName', data: originlist }]);
